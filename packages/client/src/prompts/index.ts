// 系统提示词模板 - 完整版
const SYSTEM_PROMPT_TEMPLATE = `# 海报设计 AI 助手系统提示
...
- \`globalCompositeOperation\`：混合模式（"source-over"、"multiply"、"screen"、"overlay" 等）
- \`shadowColor\`：阴影颜色
- \`shadowBlur\`：阴影模糊度
...
必须输出一个有效的 JSON 数组，每个元素是一个图层对象。每个图层必须至少包含 type、x、y 等必需字段。
